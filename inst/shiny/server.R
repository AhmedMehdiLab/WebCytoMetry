...
server <- function(input, output, session) {
  ...
  # load essential data
  flow_item <- reactive({req(input$home_source); cache$items[[input$home_source]]})
  flow_proc <- reactive({rescale_item(flow_item(), db_home_trans(), input$trans_a, input$trans_b, input$trans_c)})

  ch_antigen <- reactive({setNames(flow_item()$panel$antigen, flow_item()$panel$external)})
  ch_colname <- reactive({setNames(flow_item()$panel$fcs_colname, flow_item()$panel$external)})
  ...
  db_heat_expr <- reactive({input$heat_expr}) %>% debounce(1000)
  ...
  expr_info <- reactive({collate_expressions(flow_proc(), min_cells = input$files_min)})
  ...
  # cluster data
  cluster <- reactive({
    validate(need(length(db_som_channels()) > 1, "Select two or more channels for clustering"))
  ...
  xprc_info <- reactive({collate_expressions(flow_proc(), cluster()$sce, input$files_min)})
  xpsc_info <- reactive({rescale_expressions(xprc_info(), input$xp_range)})

  # analyze clustered expression data
  glmm <- reactive({compute_glmm(xprc_info(), input$som_meta, input$glmm_min) %>% compute_results()})
  lmer <- reactive({compute_lmer(xprc_info(), input$som_meta, input$lmer_min) %>% compute_results()})
  radviz <- reactive({
  ...
  output$plot_abnd <- renderPlot({CATALYST::plotAbundances(cluster()$sce, input$som_meta, input$abnd_mode, check(input$abnd_group), check(input$abnd_shape))})
  output$plot_code <- renderPlot({CATALYST::plotCodes(cluster()$sce, input$som_meta)})
  output$plot_clxp <- renderPlot({CATALYST::plotClusterExprs(cluster()$sce, input$som_meta, NULL)})
  output$plot_heat <- renderPlot({
    validate(need(!is.null(db_heat_expr()), "Select one or more channels"))
    CATALYST::plotMultiHeatmap(cluster()$sce, db_heat_expr(), input$heat_freq, input$som_meta, bars = TRUE, perc = TRUE)
  })
